...
for name in names:
    ...
    for iterations in range(1, max_value + 1):
        logic = f"{name}{iterations}"
        #Field Condition Name
        condition_select = "//select[@name='rule_id']"
        driver.find_element(By.XPATH, condition_select).click()
        condition_options = f"//select[@name='rule_id']/option[text()='Actions#7__{logic}']"
        driver.find_element(By.XPATH, condition_options).click()
        time.sleep(2)
        ...
        for role in range(1, number_of_role + 1):
            ...
            match combi:
                ...
                case "A2-1":
                    driver.find_element(By.NAME, "not_used_checkbox397").click()
                    action = ActionChains(driver)
                    action.key_down(Keys.CONTROL).click(option1_user).click(option2_user).click(option3_user).key_up(Keys.CONTROL).perform()
                    time.sleep(2)
                    break
                case "A2-2":
                    driver.find_element(By.NAME, "not_used_checkbox398").click()
                    action = ActionChains(driver)
                    action.key_down(Keys.CONTROL).click(option1_tm).click(option2_tm).click(option3_tm).key_up(Keys.CONTROL).perform()
                    time.sleep(2)
                    break 
